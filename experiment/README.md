...
`experiment.generate_second_order_20k` is the frozen single-B200 second-order pipeline. Adapter/base loading and user-only prompt rendering come only from `evaluate_llama_adapter.py`; the immutable layout, length sort, tokenizer padding, scheduling, decode, allocator cleanup, retry, pressure, resume, and original-order export semantics are directly bound to `generate_teacher_20k.py` (source SHA-256 `20334a6d1f3c3140f6ea359eb33f49f2e55a218067d2b26f8553f765ae199811`). It validates the authoritative 20,000-row input SHA-256 `b404c94e81510d5b17e3f04df38d7905aa639cbe0343db0fc925a317164dee90`, renders/tokenizes all prompts into one immutable layout, processes deterministic `(input_tokens, original_index)` work, and restores original order for the final five-key corpus.

The four-RTX-PRO-4500/four-shard proposal remains cancelled, not a compatibility path. Its retained `second-order-llama20k-seed42-20260830T012632Z` diagnostic is non-authoritative and must not be resumed or merged. The later `second-order-llama20k-b200-seed42-20260830T021033Z` is also invalid diagnostic-only: forced mixed representative/longest batches 512, 256, and 128 OOMed; its 64 command was aborted; no accepted gate or output exists; and the pod was deleted.

Smoke and formal execution require exactly one visible `NVIDIA B200`, one model process, and no tensor parallelism. Scheduler maximum begins and is capped at 512; it is not an actual-group promise. Each sorted pending prefix uses the largest group at or below that maximum satisfying `actual_size * padded_input_tokens <= 131072` (so a 7,217-token prefix schedules about 18 while short prefixes schedule 512). Each smoke attempt runs exactly the first group chosen by that same full-layout scheduler and writes its own immutable `schedule.json`, whose SHA-256 is bound by the report and accepted gate; a failed 512 attempt therefore cannot conflict with a distinct 256 attempt. Only formal writes `formal-schedule.json`, using the accepted maximum. OOM or allocated pressure at/above 0.92 halves the next maximum; a successful below-threshold smoke accepts its current maximum (512 initially). The maximum never grows or brackets. Every attempted batch resets seed 42, raw decode preserves bytes without `.strip()`, batches are ordinal, and resume requires the exact sorted-work prefix plus durable scheduler evidence reconstructed from the accepted initial maximum.

Prepare, then run exactly one controller action at a time; B200 availability is not implied:

```powershell
.\scripts\generate-second-order-20k.ps1 -Prepare -RunRoot /workspace/runs/second-order-llama-20k-YYYYMMDDTHHMMZ -InputPath /workspace/runs/abliterated-20000-20260829T022737Z/output/rollouts.jsonl -Checkpoint /workspace/runs/llama-abliterated-seed42-1ep-20260829T051410Z/checkpoints/step-000157 -StagingManifest /workspace/runs/model-staging-provenance-20260826T2347Z/model-manifest.json
.\scripts\generate-second-order-20k.ps1 -Smoke -BatchSize 512 -RunRoot /workspace/runs/second-order-llama-20k-YYYYMMDDTHHMMZ -InputPath /workspace/runs/abliterated-20000-20260829T022737Z/output/rollouts.jsonl -Checkpoint /workspace/runs/llama-abliterated-seed42-1ep-20260829T051410Z/checkpoints/step-000157 -StagingManifest /workspace/runs/model-staging-provenance-20260826T2347Z/model-manifest.json
...
The controller remains Windows PowerShell 5.1 ASCII, resolves the exact configured SSH-ready pod through `lib.ps1`, changes only to `/workspace/code/mats12`, uses `/root/mats12-second-order-venv/bin/python`, passes `InputPath` (with `Alias('Input')`), redirects Ubuntu stderr with `2>&1`, and performs no local Python, `nvidia-smi`, provisioning, or deletion. `-Start` makes one durable intent and one disconnect-safe supervisor; monitor reconciles terminal, exit, and PID-start identity evidence. Finalization atomically fsyncs exactly one five-key original-order 20,000-row corpus and canonical root `DONE`.
