...
`experiment.generate_second_order_20k` is the frozen single-B200 second-order pipeline. It validates the authoritative 20,000-row input SHA-256 `b404c94e81510d5b17e3f04df38d7905aa639cbe0343db0fc925a317164dee90`, projects only `id`, `source`, and `prompt` in original order, and materializes one immutable 20,000-row prompt stream before model loading. It preserves the completed seed-42 adapter/staging/evaluator renderer, left padding, continuation slicing, and one BF16 sampled response per prompt (`temperature=1`, `top_p=1`, `top_k=0`, `max_new_tokens=4096`).

The four-RTX-PRO-4500/four-shard proposal is cancelled, not retained as a compatibility path. The preserved run `second-order-llama20k-seed42-20260830T012632Z` is non-authoritative diagnostic-only: attempts 256, 128, 64, 32, and 16 OOMed; its batch-8 command was aborted; no accepted gate/output exists; and the pod was deleted. Do not resume or merge it.

Smoke and formal execution require exactly one visible `NVIDIA B200`, one model process, no tensor parallelism, and one direct global stream `0..19999`. Smoke starts at 512 and uses immutable ordinal attempt directories. Existing thresholds can halve/double the next recommendation; an accepted bracket may reference a prior successful lower attempt and a later limiting upper attempt. Formal start requires that terminal gate and its exact batch size. It begins at the accepted size; an OOM before publication may halve the remainder after CUDA tensor cleanup, preserving global-start-based batch names and batch-layout-dependent `42 + global batch start` seeds. Every raw row is rebound to its exact authoritative row. Resume accepts only a verified direct immutable prefix.
