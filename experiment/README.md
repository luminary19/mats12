...
The four-RTX-PRO-4500/four-shard proposal remains cancelled. The retained `second-order-llama20k-seed42-20260830T012632Z`, `second-order-llama20k-b200-seed42-20260830T021033Z`, and `second-order-llama20k-b200-lengthsorted-seed42-20260830T030933Z` runs are cancelled diagnostic-only evidence; none has an output and none may be resumed or merged.

Formal execution starts directly after prepare and requires exactly one visible `NVIDIA B200`, one model process, and no tensor parallelism. It starts at batch size 512 and selects exactly `pending[:current_batch_size]`, regardless of prompt length; tokenizer padding applies only to that selected group. On a pre-publication OOM, cleanup runs and the next size is exactly `max(1, floor(current * 3 / 4))` (512, 384, 288, 216, 162); successful memory pressure is diagnostic only and never changes size. Every attempted batch resets seed 42, raw decode preserves bytes without `.strip()`, and resume validates journaled exact sorted-work prefixes and transitions. The first published ordinal batch is the launch probe: monitor, supervisor logs, and its immutable batch manifest/data checksum provide launch evidence without a separate generation.
...
.\scripts\generate-second-order-20k.ps1 -Monitor -BatchSize 512 -RunRoot /workspace/runs/second-order-llama-20k-YYYYMMDDTHHMMZ -InputPath /workspace/runs/abliterated-20000-20260829T022737Z/output/rollouts.jsonl -Checkpoint /workspace/runs/llama-abliterated-seed42-1ep-20260829T051410Z/checkpoints/step-000157 -StagingManifest /workspace/runs/model-staging-provenance-20260826T2347Z/model-manifest.json
.\scripts\generate-second-order-20k.ps1 -Finalize -BatchSize 512 -RunRoot /workspace/runs/second-order-llama-20k-YYYYMMDDTHHMMZ -InputPath /workspace/runs/abliterated-20000-20260829T022737Z/output/rollouts.jsonl -Checkpoint /workspace/runs/llama-abliterated-seed42-1ep-20260829T051410Z/checkpoints/step-000157 -StagingManifest /workspace/runs/model-staging-provenance-20260826T2347Z/model-manifest.json
```
