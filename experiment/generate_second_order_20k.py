"""Generate the immutable second-order Llama corpus in direct formal batches.

Plan mode is standard-library only. Formal execution is limited to one visible B200,
one BF16 adapter-loaded model process, and no tensor parallelism.
"""
...
# Decode and allocator behavior are bound to the authoritative 9B 20k generator.
...
def validate_amendment(path: Path) -> dict[str, Any]:
    ...
    if (value.get("format") != "second-order-llama-adapter-20000-amendment-v4"
            ...
            or execution != {"gpu_name": GPU_NAME, "visible_cuda_gpus": 1, "model_processes": 1,
                             "tensor_parallel": False, "initial_batch_size": MAX_BATCH_SIZE,
                             "oom_transition": "max(1, floor(current_batch_size * 3 / 4))",
                             "successful_memory_pressure_is_diagnostic_only": True}
...
def _plan_manifest(args: argparse.Namespace, prompts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {"format": "second-order-llama-adapter-20k-v4", "run_id": Path(args.run_root).name,
            ...
            "batching": {"initial_batch_size": MAX_BATCH_SIZE, "current_batch_size": "resume from journal",
                         "selection": "pending[:current_batch_size]", "oom_transition": "max(1, floor(current_batch_size * 3 / 4))",
                         "successful_memory_pressure": "diagnostic_only", "monotonically_non_increasing": True},
...
def _next_batch_size_after_oom(current: int) -> int:
...
def _batch_manifest(group: Sequence[Mapping[str, Any]], padded: int, details: Mapping[str, Any], before: int,
                    ordinal: int, rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {"batch_ordinal": ordinal, "actual_size": len(group), "padded_input_tokens": padded,
            ...
            "batch_size_before": before, "batch_size_after": before, "batch_seed": MASTER_SEED,
...
def _worker_rows(run: Path, prompts: Sequence[Mapping[str, Any]], plan_sha: str,
    ...
    for ordinal, batch in enumerate(batches):
        ...
        if (manifest.get("plan_sha256") != plan_sha or manifest.get("batch_ordinal") != ordinal
                or manifest.get("actual_size") != len(data) or manifest.get("batch_seed") != MASTER_SEED
                or not isinstance(manifest.get("batch_size_before"), int) or manifest["batch_size_before"] < 1
                or manifest.get("batch_size_after") != manifest["batch_size_before"]):
            raise ValidationError("batch size evidence differs")
        if (len(data) > manifest["batch_size_before"]
                or manifest["padded_input_tokens"] != max(row["prompt_tokens"] for row in data)):
            raise ValidationError("batch selection evidence differs")
    ...
    current, published_ordinal, pending_offset, prior_attempt = MAX_BATCH_SIZE, 0, 0, None
    for event in events:
        name = event.get("event")
        if name == "attempt":
            if (event.get("current_batch_size") != current or event.get("batch_ordinal") != published_ordinal
                    or event.get("seed") != MASTER_SEED):
                raise ValidationError("batch attempt evidence differs")
            if work is not None:
                group = list(work[pending_offset:pending_offset + current])
                padded = max(row["input_tokens"] for row in group) if group else 0
                if (not group or event.get("actual_size") != len(group) or event.get("padded_input_tokens") != padded
                        or event.get("original_indices") != [row["global_index"] for row in group]):
                    raise ValidationError("attempt does not bind the next exact sorted-work prefix")
            if prior_attempt is not None and event != prior_attempt:
                raise ValidationError("repeated dangling attempt differs from the exact retryable attempt")
            prior_attempt = event
        elif name == "oom_before_publish":
            next_current = _next_batch_size_after_oom(current)
            if (prior_attempt is None or event.get("batch_ordinal") != published_ordinal
                    or event.get("current_batch_size_before") != current
                    or event.get("current_batch_size_after") != next_current
                    or event.get("actual_size") != prior_attempt.get("actual_size")):
                raise ValidationError("OOM batch-size reduction differs from exact three-quarter transition")
            current, prior_attempt = next_current, None
        elif name == "published":
            ...
            if (event.get("batch_ordinal") != published_ordinal or event.get("current_batch_size") != current
                    or manifest["batch_size_before"] != current or manifest["actual_size"] != event.get("actual_size")
                    or manifest["batch_size_after"] != current):
                raise ValidationError("published batch-size evidence differs")
    ...
    # One exact trailing attempt is retryable after abrupt interruption; it has no output and changes no state.
    return rows, current
...
def worker(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with RunHeartbeat(run) as heartbeat:
        ...
        worker_manifest = {"format": "second-order-worker-record-v5", "plan_sha256": plan_sha,
                           "prompt_layout_sha256": layout_sha, "initial_batch_size": MAX_BATCH_SIZE,
                           "oom_transition": "max(1, floor(current_batch_size * 3 / 4))",
        ...
        existing, current = _worker_rows(run, prompts, plan_sha, work)
        pending, ordinal = work[len(existing):], len(finalized_batches(run / "raw" / "batches"))
        while pending:
            group = list(pending[:current])
            padded = max(row["input_tokens"] for row in group)
            before = current
            _append_scheduler_event(run, event="attempt", batch_ordinal=ordinal, current_batch_size=before,
                                    actual_size=len(group), padded_input_tokens=padded,
                                    original_indices=[row["global_index"] for row in group], seed=MASTER_SEED)
            try:
                generated, details = _attempt_after_allocator_cleanup(
                    torch, lambda: _generate_attempt(torch, tokenizer, model, group, padded))
            except BaseException as exc:
                if not _is_oom(torch, exc): raise
                current = _next_batch_size_after_oom(before)
                _append_scheduler_event(run, event="oom_before_publish", batch_ordinal=ordinal,
                                        current_batch_size_before=before, current_batch_size_after=current,
                                        actual_size=len(group), padded_input_tokens=padded,
                                        original_indices=[row["global_index"] for row in group], error_type=type(exc).__name__)
                heartbeat.write_metric(event="oom_before_publish", current_batch_size_before=before,
                                       current_batch_size_after=current, actual_size=len(group))
                del exc; _release_cuda_allocator_cache(torch)
                if before == 1: raise ValidationError("one-row batch OOM cannot be safely reduced")
                continue
            raw = _raw_with_ordinal(generated, ordinal); _validate_raw(raw, prompts)
            manifest_extra = _batch_manifest(group, padded, details, before, ordinal, raw)
            ...
            _append_scheduler_event(run, event="published", batch=batch_name, batch_ordinal=ordinal,
                                    current_batch_size=current, actual_size=len(raw), padded_input_tokens=padded,
                                    original_indices=[row["global_index"] for row in group], seed=MASTER_SEED)
            heartbeat.write_metric(event="batch_published", batch=batch_name, current_batch_size=current,
                                   actual_size=len(raw), launch_probe=(ordinal == 0),
                                   sha256=sha256_file(final / "data.jsonl"), **details)
            pending, ordinal = pending[len(group):], ordinal + 1
        rows, reconstructed = _worker_rows(run, prompts, plan_sha, work)
        if len(rows) != EXPECTED_ROWS or reconstructed != current: raise ValidationError("formal coverage or batch-size reconstruction incomplete")
        record = {"format": "second-order-worker-record-v5", "plan_sha256": plan_sha, "row_count": len(rows),
                  "current_batch_size": current, "raw_sha256": sha256_text(json.dumps(rows, ensure_ascii=False, separators=(",", ":"))),
...
def start(args: argparse.Namespace) -> dict[str, Any]:
    ...
    intent = {"format": "second-order-launch-intent-v4", "plan_sha256": plan_sha,
              "initial_batch_size": MAX_BATCH_SIZE, "gpu_name": GPU_NAME, "visible_cuda_gpus": 1, "model_processes": 1,
              "first_published_batch_is_launch_probe": True}
...
def finalise(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with RunHeartbeat(run) as heartbeat:
        ...
        output_manifest = {"format": "second-order-five-key-rollouts-v4", "row_count": count, "sha256": digest, "schema": list(ROW_KEYS),
    ...
    mark_done(root, {"status": "DONE", "format": "second-order-canonical-root-v4", "plan_sha256": plan_sha,
                     "initial_batch_size": MAX_BATCH_SIZE, "final_output_sha256": digest,
