...
`protocol-amendments/second-order-llama-adapter-20000-2026-08-30.json` v4 freezes the direct formal second-order corpus. It retains the completed seed-42 step-157 adapter, authoritative 20,000-row input, and evaluator-only adapter/base loading and rendering. It retains `generate_teacher_20k.py` decode, tokenizer-padding, allocator-cleanup, seed-reset, immutable publication, resume, and final-export semantics, but intentionally replaces its token-budget scheduler with the direct formal batch policy below.

The four-`NVIDIA RTX PRO 4500 Blackwell`/four-shard design remains cancelled. The retained RTX diagnostic, mixed-batch B200 diagnostic `second-order-llama20k-b200-seed42-20260830T021033Z`, and cancelled pre-formal smoke-only `second-order-llama20k-b200-lengthsorted-seed42-20260830T030933Z` are non-authoritative and must not be resumed, merged, or treated as gates.

Formal starts directly after prepare on one visible `NVIDIA B200`, one model process, no shards or tensor parallelism. Render/tokenize all prompts, sort by `(input_tokens, original_index)`, begin at 512, and select exactly `pending[:current_batch_size]` (or final remainder), independent of length. Padding is only over that selected group. There is no token budget, prompt-length scheduler, smoke gate, or separate probe. A pre-publication OOM alone changes current size to `max(1, floor(current * 3 / 4))`: 512→384→288→216→162. Successful memory metrics are diagnostic only; current remains unchanged.

Generation remains one BF16 sampled response per prompt with seed 42 reset for every attempted batch. Decode preserves bytes exactly; EOS is counted but not decoded. Batch names are ordinals. Resume validates every journaled three-quarter OOM transition, every selected/published group as the next exact length-sorted prefix with `actual_size=min(current, remaining)`, and resumes an unpublished OOM at that same prefix. The first immutable published batch is the launch probe; monitor, supervisor log, batch manifest, and checksum confirm launch. Finalization revalidates identities and atomically restores original index order.

The Windows PowerShell 5.1 controller is a remote-only controller; every invocation takes exactly one action (`-Prepare`, `-Start`, `-Monitor`, or `-Finalize`) and uses `/workspace/code/mats12`, the ephemeral `/root/mats12-second-order-venv/bin/python`, `InputPath` with `Alias('Input')`, and Ubuntu `2>&1`. It never provisions, deletes, or inspects a provider locally. `-Start` records one durable intent and launches one disconnect-safe supervisor; duplicate starts are refused, monitor reconciles DONE/CRASHED/PID-start identity/exit evidence, and partial-start rollback leaves no worker leak. This amendment does not assert that B200 capacity is currently available.
