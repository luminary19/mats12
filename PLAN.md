...
`protocol-amendments/second-order-llama-adapter-20000-2026-08-30.json` authorizes a distinct second-order inheritance corpus. It binds the completed seed-42 step-157 adapter, the exact five-key abliterated 20,000-row rollout SHA-256 `b404c94e81510d5b17e3f04df38d7905aa639cbe0343db0fc925a317164dee90`, the base/tokenizer staging identities, and the frozen evaluator renderer: a user-only HF chat template with `date_string="27 Aug 2026"` plus its legacy extra BOS. Only input `id`, `source`, and `prompt` are projected; the prior teacher response is never model input. Every prompt must fit with the fixed 4,096 generated-token allowance or the run fails closed.

The corpus has exactly one sampled BF16 completion per prompt, `do_sample=true`, temperature 1, top-p 1, top-k 0, and max-new-tokens 4,096. It is materialized before inference as four immutable physical contiguous prompt shards `[0,5000)`, `[5000,10000)`, `[10000,15000)`, and `[15000,20000)`. Formal inference is exactly four independent processes, each seeing one RTX PRO 4500 Blackwell through `CUDA_VISIBLE_DEVICES` and loading one whole active PEFT model; tensor parallelism, quantization, CPU offload, and remote-code loading are prohibited. A 256-prompt one-GPU deterministic representative/stress smoke precedes formal work and reports allocator baseline/peaks/pressure, free memory, throughput, prompt lengths, terminations, blanks, OOM evidence, and a bounded power-of-two recommendation. It does not start formal inference.

A formal OOM may halve an uncommitted batch after allocator cleanup, but never mutates a published batch. The batch seed is `42 + global batch start index`, reset in Python/Torch/CUDA immediately before each attempt. Consequently the outputs are deterministic for their recorded shard/batch layout only; this protocol does not claim row-level RNG independence. Raw immutable rows retain index, prompt/response hashes, adapter hashes, shard/batch/seed identities, unpadded/padded token counts, output count, termination, and blank status. After semantically validating all four terminal shards, finalization atomically writes a five-key, original-order training output. The PowerShell 5.1 launcher has no provider lifecycle operation.
