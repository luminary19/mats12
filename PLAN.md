...
`protocol-amendments/second-order-llama-adapter-20000-2026-08-30.json` freezes a distinct second-order inheritance corpus. It binds the completed seed-42 step-157 adapter, the authoritative five-key 20,000-row rollout SHA-256 `b404c94e81510d5b17e3f04df38d7905aa639cbe0343db0fc925a317164dee90`, staging identities, and the evaluator's user-only HF renderer with date `27 Aug 2026` and legacy extra BOS. It projects only `id`, `source`, and `prompt` from all 20,000 authoritative rows in their original order; the original response is never model input.

The previously proposed four-`NVIDIA RTX PRO 4500 Blackwell`/four-shard design is explicitly cancelled and superseded before formal inference. The retained RTX smoke run `second-order-llama20k-seed42-20260830T012632Z` is non-authoritative diagnostic evidence only: attempts 256, 128, 64, 32, and 16 all OOM; its batch-8 command was aborted; it produced neither an accepted smoke gate nor output; and its pod was deleted. It must not be resumed, merged, or treated as a gate.

The new protocol requires exactly one visible `NVIDIA B200`, exactly one model process, and one direct ordered stream of global indices `0..19999`; there are no shards, GPU `0..3` assignments, coordinator workers, or tensor parallelism. It begins smoke at batch 512. Ordinal immutable smoke attempts obey the existing pressure/OOM thresholds: a successful stable attempt may accept itself; a later limiting upper attempt may bracket and accept the prior successful lower attempt. Once terminal, the chain cannot oscillate or repeat. Formal inference may start only with the accepted exact batch size, uses it as the initial batch size, and processes global indices sequentially in immutable batches. A pre-publication OOM may halve only the remaining batch; tensor cleanup precedes the retry and each resulting layout-dependent `42 + global batch start` seed is retained.

Generation is one BF16 sampled response per prompt (`temperature=1`, `top_p=1`, `top_k=0`, `max_new_tokens=4096`). Every immutable raw row is rebound to its exact authoritative global index/id/source/prompt/hash, not merely self-hashed. It retains adapter/staging/source/runtime/Git identity, left-padding and continuation evidence, stripped response, token/termination/blank fields, allocator and throughput metrics. Resume validates the direct immutable prefix before model load. Finalization validates the one completed worker record, atomically fsyncs one original-order five-key 20,000-row corpus, and writes canonical root `DONE`.
