...
`protocol-amendments/second-order-llama-adapter-20000-2026-08-30.json` v3 freezes a distinct second-order inheritance corpus. It binds the completed seed-42 step-157 adapter, the authoritative five-key 20,000-row rollout SHA-256 `b404c94e81510d5b17e3f04df38d7905aa639cbe0343db0fc925a317164dee90`, and the evaluator's user-only renderer with date `27 Aug 2026` and legacy extra BOS. It borrows adapter/base loading and rendering only from `experiment/evaluate_llama_adapter.py` (SHA-256 `9e8b049529bc4bbb2b64ab51a3495b11589ef30c859de562306830ffbf628aa7`). The scheduler, padding, generation/decode, allocator cleanup, OOM/pressure reduction, resume, and final export are adapted directly from `experiment/generate_teacher_20k.py` (SHA-256 `20334a6d1f3c3140f6ea359eb33f49f2e55a218067d2b26f8553f765ae199811`). It projects only `id`, `source`, and `prompt`; original responses are never model input.

The previously proposed four-`NVIDIA RTX PRO 4500 Blackwell`/four-shard design remains explicitly cancelled. The retained RTX smoke run `second-order-llama20k-seed42-20260830T012632Z` is non-authoritative diagnostic evidence only and must not be resumed, merged, or treated as a gate. The later B200 run `second-order-llama20k-b200-seed42-20260830T021033Z` is also invalid diagnostic-only: forced mixed representative/longest 512/256/128 batches OOMed, its 64 command was aborted, no accepted gate/output exists, and the pod was deleted.

The corrected protocol requires one visible `NVIDIA B200`, one model process, no shards or tensor parallelism. Render/tokenize all prompts first into an immutable layout, sort pending work by `(input_tokens, original_index)`, and use the direct 20k `_schedule_batch` rule: choose the largest pending prefix no larger than the current scheduler maximum for which `actual_size * padded_input_tokens <= 131072`. The scheduler maximum begins and is capped at 512, is monotonically non-increasing, and is distinct from actual group size: short work may use 512 while 7,217-token prompts use about 18. Each smoke attempt builds that same full layout and generates only the first scheduled group. It writes an attempt-local immutable schedule and binds its SHA-256, maximum, actual size, padded length, prompt-layout SHA, budget product, memory, throughput, and output evidence; only formal writes the root `formal-schedule.json` for the accepted maximum. OOM or allocated pressure at/above 0.92 halves the next maximum; below threshold accepts the current successful maximum (512 initially). There is no doubling or bracket search.

Generation is one BF16 sampled response per prompt (`temperature=1`, `top_p=1`, `top_k=0`, `max_new_tokens=4096`). Master seed 42 resets before every attempted batch and outputs are explicitly batch-layout-dependent. EOS is counted but not decoded; decoded response bytes are retained without `.strip()` (blank detection alone may strip). Batch names are ordinals, evidence records sorted-work original indices and prompt identities, and resume accepts only an exact prefix of deterministic length-sorted work while reconstructing its non-increasing maximum from immutable batch/journal evidence beginning at the accepted smoke maximum. Offline schedule simulation proves every planned group meets budget and covers all 20,000 identities exactly once. Finalization revalidates identities, restores original index order, atomically fsyncs the five-key corpus, and writes canonical root `DONE`.

The Windows PowerShell 5.1 controller is a remote-only controller; every invocation takes exactly one action (`-Prepare`, `-Smoke`, `-Start`, `-Monitor`, or `-Finalize`) and uses `/workspace/code/mats12`, the ephemeral `/root/mats12-second-order-venv/bin/python`, `InputPath` with `Alias('Input')`, and Ubuntu `2>&1`. It never provisions, deletes, or inspects a provider locally. `-Start` records one durable intent and launches one disconnect-safe supervisor; duplicate starts are refused, monitor reconciles DONE/CRASHED/PID-start identity/exit evidence, and partial-start rollback leaves no worker leak. This amendment does not assert that B200 capacity is currently available.
