...
function Invoke-ExactRunMirror($Pod,[string[]]$RunIds) {
    ...
    try {
        foreach($id in $RunIds) {
            ...
            $command="cd $(ConvertTo-PosixSingleQuoted $remote) && find . -type f -printf '%P\0' | sort -z | while IFS= read -r -d '' p; do printf '%s\t%s\t%s\n' \"`$p\" \"`$(stat -c %s -- \"`$p\")\" \"`$(sha256sum -- \"`$p\" | cut -d ' ' -f1)\"; done"
            ...
            foreach($line in $inventory.Lines){$parts=$line -split "`t",3;if($parts.Count-ne3 -or -not(Test-SafeRelPath $parts[0]) -or $parts[1] -notmatch '^\d+$' -or $parts[2] -notmatch '^[0-9a-f]{64}$'){throw 'Invalid exact inventory row.'};$local=Join-Path $target ($parts[0]-replace '/','\');$parent=Split-Path $local -Parent;New-Item -ItemType Directory -Path $parent -Force|Out-Null;$temp=Get-LocalAtomicTempPath $local;if(-not(Copy-FileFromPod $Pod ($remote+'/'+$parts[0]) $temp)){throw 'Exact mirror download failed.'};if((Get-Item $temp).Length-ne[int64]$parts[1] -or ((Get-FileHash $temp -Algorithm SHA256).Hash.ToLowerInvariant()-ne$parts[2])){throw 'Exact mirror hash/size mismatch.'};Move-Item -LiteralPath $temp -Destination $local}
