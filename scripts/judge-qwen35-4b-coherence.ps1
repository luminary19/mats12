...
function Invoke-CoherenceCommand {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Qwen coherence command failed with exit code $LASTEXITCODE."
    }
}

Push-Location $repoRoot
try {
    Invoke-CoherenceCommand @('-m', 'experiment.prepare_qwen35_4b_coherence', '--prepare', '--run-dir', $RunDir)
    Invoke-CoherenceCommand @('-m', 'experiment.judge_qwen35_4b_coherence', '--plan', '--run-dir', $RunDir)
    if (-not $Execute) {
    ...
    try {
        ...
        Invoke-CoherenceCommand @('-m', 'experiment.judge_qwen35_4b_coherence', '--execute', '--run-dir', $RunDir)
    }
    finally {
