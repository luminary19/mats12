...
switch($Action){
 ...
 'Prepare' {
  $command="set -eu; cd $(Q $project); rm -rf /tmp/mats12-qwen35-4b-train-venv; python3 -m venv /tmp/mats12-qwen35-4b-train-venv; $python -m pip install --upgrade pip -q > /tmp/qwen35-install.log 2>&1; $python -m pip install -q --extra-index-url https://download.pytorch.org/whl/cu128 torch==2.8.0+cu128 torchvision==0.23.0+cu128 Pillow==11.3.0 transformers==5.16.1 accelerate==1.10.1 peft==0.18.1 safetensors==0.8.0 huggingface-hub==1.28.0 ninja packaging setuptools wheel >> /tmp/qwen35-install.log 2>&1; MAX_JOBS=8 $python -m pip install -q --no-build-isolation 'flash-linear-attention[cuda]==0.5.2' causal-conv1d==1.7.0 >> /tmp/qwen35-install.log 2>&1; $python -m pip freeze | grep -E '^(torch|torchvision|pillow|transformers|peft|accelerate|safetensors|huggingface-hub|flash-linear-attention|causal-conv1d)=='"
