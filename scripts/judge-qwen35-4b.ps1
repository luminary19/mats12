...
try {
    Invoke-JudgeCommand @('-m','experiment.judge_qwen35_4b','--plan','--run-dir',$RunDir,'--concurrency','16')
    ...
    try { [Environment]::SetEnvironmentVariable($keyName, $userKey, 'Process'); Invoke-JudgeCommand @('-m','experiment.judge_qwen35_4b','--execute','--run-dir',$RunDir,'--concurrency','16') }
