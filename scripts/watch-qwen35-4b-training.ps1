...
param(
    ...
    [Parameter(Mandatory=$true)][ValidatePattern('^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$')][string]$AcceptedResumeSmokeRunId,
...
function Invoke-ExactRunMirror($Pod,[string[]]$RunIds) {
    $stage = Join-Path $script:Sprint.LocalRuns ('.qwen35-4b-mirror-' + [Guid]::NewGuid().ToString('N'))
    $backups=@{}; $published=New-Object System.Collections.Generic.List[string]
    ...
    try {
        ...
        foreach($id in $RunIds){
            $final=Join-Path $script:Sprint.LocalRuns $id
            $candidate=Join-Path $stage $id
            if(Test-Path $final){$backup=$final+'.mirror-backup-'+[Guid]::NewGuid().ToString('N');Move-Item -LiteralPath $final -Destination $backup;$backups[$final]=$backup}
            Move-Item -LiteralPath $candidate -Destination $final
            $published.Add($final)
        }
        foreach($backup in $backups.Values){if(Test-Path $backup){Remove-Item -LiteralPath $backup -Recurse -Force -ErrorAction SilentlyContinue}}
    } catch {
        for($i=$published.Count-1;$i-ge0;$i--){$final=$published[$i];if(Test-Path $final){Remove-Item -LiteralPath $final -Recurse -Force}}
        foreach($final in $backups.Keys){$backup=$backups[$final];if(Test-Path $backup){Move-Item -LiteralPath $backup -Destination $final}}
        throw
...
if ($Action -eq 'StartDetached') {
    ...
    & $ValidatorPython -c "import experiment.train_qwen35_4b_lora_local"; if ($LASTEXITCODE -ne 0) { throw 'ValidatorPython cannot import the validator.' }
    Write-WatcherLog ('ValidatorPython=' + $ValidatorPython + '; version=' + (& $ValidatorPython --version 2>&1))
    $arguments = '-NoProfile -ExecutionPolicy Bypass -File "{0}" -Action Watch -RunId "{1}" -ExpectedPodId "{2}" -ExpectedCommit "{3}" -AcceptedSmokeRunId "{4}" -AcceptedResumeSmokeRunId "{5}" -ValidatorPython "{6}" -PollSeconds {7}' -f $scriptPath, $RunId, $ExpectedPodId, $ExpectedCommit, $AcceptedSmokeRunId, $AcceptedResumeSmokeRunId, $ValidatorPython, $PollSeconds
...
try {
    ...
    while ($true) {
        try {
            ...
            Invoke-ExactRunMirror $pod @($AcceptedSmokeRunId,$AcceptedResumeSmokeRunId,$RunId)
            $localRun = Join-Path $script:Sprint.LocalRuns $RunId
            & $ValidatorPython -m experiment.train_qwen35_4b_lora_local --validate-completed --validation-mode static --run-kind full --run-dir $localRun
